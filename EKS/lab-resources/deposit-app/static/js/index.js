$(document).ready(function() {
    ...
    $('.refresh_cust').click(function(event){
        ...
        $.ajax({
            ...
            console.log(result)
            parent_ele = target.parentElement.parentElement
            parent_ele.children[2].innerHTML = result.message
            parent_ele.children[3].innerHTML = result.date
        }).fail(function(error){
            console.log(error)
        })
    })
    ...
    $('.refresh_acc').click(function(event){
        ...
        $.ajax({
            ...
            console.log(result)
            parent_ele = target.parentElement.parentElement
            parent_ele.children[3].innerHTML = result.status
            parent_ele.children[4].innerHTML = result.message
            parent_ele.children[5].innerHTML = result.date
        }).fail(function(error){
            console.log(error)
        })
    })
